<nav class="bg-[#1c120c] text-amber-100 border-b border-amber-950/80 sticky top-0 z-50 shadow-xl">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20 items-center">
            ...
            <div class="hidden md:flex items-center space-x-7 font-bold text-xs uppercase tracking-wider text-amber-200/90">
                <a href="{{ route('home') }}" class="hover:text-amber-400 transition-colors {{ request()->routeIs('home') && !request('category') && !request('search') ? 'text-amber-400 font-extrabold border-b-2 border-amber-400 pb-1' : '' }}">
                    Beranda
                </a>
                <a href="{{ route('home') }}#kategori" class="hover:text-amber-400 transition-colors">
                    Buku Menu
                </a>
                <a href="{{ route('home') }}#resep" class="hover:text-amber-400 transition-colors">
                    Daftar Hidangan
                </a>
                <a href="{{ route('home') }}#tips" class="hover:text-amber-400 transition-colors">
                    Rahasia Dapur
                </a>
            </div>
            ...
            <div class="hidden sm:flex items-center space-x-3">
                ...
                <form action="{{ route('home') }}" method="GET" class="relative">
                    ...
                    >
                    <button type="submit" aria-label="Cari Resep" class="absolute right-3.5 top-2.5 text-amber-400/70 hover:text-amber-300 transition">
    ...
    <div id="mobile-menu" class="hidden md:hidden flex-col border-t border-amber-900/80 bg-[#1c120c] px-6 py-5 space-y-4 shadow-2xl">
        ...
        <div class="flex flex-col space-y-3 font-bold text-amber-200 text-xs uppercase tracking-wider">
            ...
            <a href="{{ route('home') }}#kategori" class="hover:text-amber-400 py-1">Buku Menu</a>
            <a href="{{ route('home') }}#resep" class="hover:text-amber-400 py-1">Daftar Hidangan</a>
            <a href="{{ route('home') }}#tips" class="hover:text-amber-400 py-1">Rahasia Dapur</a>
        </div>
        ...
        @if(session('is_admin'))
            <a href="{{ route('recipes.create') }}" class="block text-center bg-gradient-to-r from-amber-500 to-orange-600 text-white text-xs uppercase font-extrabold py-3 rounded-xl shadow transition tracking-wider">
                <i class="fa-solid fa-plus mr-1"></i> Tulis Resep Baru (Admin)
            </a>
